#ifndef BRK_INTERFACT_NETWORK_INTERFACE_H_
#define BRK_INTERFACT_NETWORK_INTERFACE_H_
...
#define MESSAGE_HEADER_LEN 10 //4字节(FBEB) + 2字节(事件ID) + 4字节(数据长度) = 10
...
typedef struct _ConnectSession {
	...
	u16 eid; //保存当前请求的事件 id
	i32 fd; //保存当前传送的文件句柄

	struct bufferevent* bev;

	char* read_buf; //保存读消息的缓存区
...
public:
	...
	static void listener_cd(struct evconnlistener* listener, evutil_socket_t fd,
...
#endif // !BRK_INTERFACT_NETWORK_INTERFACE_H_